  ...
  public static void main(String[] args) {
    ...
    while (!pq.isEmpty()) {
      ...
      for (Node n : cur.neighbours.keySet()) {
        Double newCost = cur.distCost + cur.neighbours.get(n);
        if (newCost < visited.getOrDefault(n.id, Double.MAX_VALUE)) {
          visited.put(n.id, newCost);
          n.distCost = newCost;
          pq.offer(n);
    ...
    System.out.println("Shortest Dist Cost: " + df.format(graph.get(goal).distCost));
